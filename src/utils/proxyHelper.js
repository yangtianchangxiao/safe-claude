...
class ProxyHelper {
  /**
   * 获取系统代理 Agent（从环境变量 HTTP_PROXY/HTTPS_PROXY/ALL_PROXY 读取）
   ...
   */
  static getSystemProxyAgent(options = {}) {
    const proxyUrl =
    ...
    try {
      ...
      if (agent) {
        logger.debug(
          `🌐 Using system proxy: ${proxyConfig.type}://${proxyConfig.host}:${proxyConfig.port}`
        )
      }
      return agent
    } catch (error) {
      logger.error('❌ Failed to parse system proxy URL:', proxyUrl, error.message)
      return null
    }
  }
