...
async function setup() {
  ...
  try {
    ...
    // 4. 创建初始化完成标记文件（使用哈希密码）
    const saltRounds = 12 // 使用更高的盐值轮数提升安全性
    const passwordHash = await bcrypt.hash(adminPassword, saltRounds)

    const initData = {
      initializedAt: new Date().toISOString(),
      adminUsername,
      passwordHash, // 存储哈希值而不是明文
      version: '1.0.0'
    }
    ...
    console.log('   docker compose up -d   - 使用 Docker Compose 启动\n')

    console.log(chalk.blue('🌐 访问地址：\n'))
    console.log(`   Web管理界面: http://localhost:${config.server.port}/admin-next/`)
...
function checkInitialized() {
  ...
  if (fs.existsSync(initFile)) {
    ...
    console.log('   3. 使用 npm start 或 docker compose restart 重启服务\n')
    return true
  }
  return false
}
